...
def get_mean(signal, fs, start_time=0, end_time=None, plot = False):
    ...
    if plot:
        # Plot the signal with its mean
        plt.figure(figsize=(10, 4))
        plt.plot(time, signal_segment, label='Signal', color='blue')
        plt.axhline(mean_value, color='red', linestyle='--', label=f'Mean: {mean_value:.2f}')
        plt.xlabel('Time (s)')
        plt.ylabel('Amplitude')
        plt.title('Signal with Mean')
        plt.legend()
        plt.grid(True)
        plt.show()
...
class Contraction:
    ...
    def check(self):
        """
        Check if the contraction meets the criteria of duration > 45 seconds.

        Returns:
        - True if duration > 45, else False.
        """
...
def plot_fhr_with_decelerations(fhr, fs, decelerations):
    ...
    plt.plot(time, fhr, label='FHR', color='blue')
    plt.plot(time, baseline, label='Baseline (Moving Avg)', color='green', linestyle='--')

    # Highlight decelerations
    for decel in decelerations:
        ...
        plt.fill_between(t_segment, fhr_segment, baseline_segment, 
                         where=fhr_segment < baseline_segment,
                         color='red', alpha=0.3)

        # Mark start, nadir, and end
        plt.plot(decel.start_time, fhr[decel.start_idx], 'ro', label='Start' if decel == decelerations[0] else "")
        plt.plot(decel.nadir_time, decel.nadir_value, 'ko', label='Nadir' if decel == decelerations[0] else "")
        plt.plot(decel.end_time, fhr[decel.end_idx], 'ro', label='End' if decel == decelerations[0] else "")

    plt.xlabel('Time (s)')
    plt.ylabel('FHR (BPM)')
    plt.title('Fetal Heart Rate with Decelerations')
...
def plot_cu_with_contractions(cu, fs, contractions):
    ...
    # Plot the CU signal
    plt.figure(figsize=(12, 6))
    plt.plot(time, cu, label='CU Signal', color='blue')

    # Highlight contraction areas
    for contr in contractions:
        # Fill the area under the contraction
        plt.fill_between(
            time[contr.start_idx:contr.end_idx + 1],
            cu[contr.start_idx:contr.end_idx + 1],
            color='red',
            alpha=0.3
        )

        # Plot start, acme, and end points
        plt.plot(
    ...
    plt.xlabel('Time (s)')
    plt.ylabel('CU Amplitude')
    plt.title('CU Signal with Detected Contractions')
    plt.legend(['CU Signal', 'Contraction Area'], loc='upper right')
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
def get_contractions(uc, fs, verbose = False):
...
def get_classified_decelerations(fhr, uc, fs, verbose = False):
    ...
    return early_decs, late_decs, variable_decs, decelerations, contractions, paired_events, mean_fhr
